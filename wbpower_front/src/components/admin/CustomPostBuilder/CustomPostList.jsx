...
const handleDelete = async (id) => {
  ...
  try {
    const res = await api.delete(`/custom-post/delete/${tableName}/${id}`);

    // Check success from Laravel response
    if (res.data.success) {
    ...
    // Handle unauthorized (token expired) or other errors
...
const toggleStatus = async (id, currentStatus) => {
  ...
  try {
    const res = await api.put(`/custom-post/status/${tableName}/${id}`, {
      is_active: newStatus,
    });
